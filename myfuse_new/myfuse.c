...
void replace_inode_cache(unsigned int inodehash,const char *fullpath, ino_t st_ino)
 {
  printf("debug replacing icache %d with %si, inode %ld\n",inodehash, fullpath, st_ino);
      ...
      if (strcmp(subname,tempent.name)==0)
       {
        ...
        if (strlen(fullpath)<MAX_ICACHE_PATHLEN)
         replace_inode_cache(inodehash,fullpath,tempent.st_ino);
    ...
    for(count=0;count<num_subdir;count++)
     {
      ...
      if (strcmp(subname,tempent.name)==0)
       {
        fclose(fptr);
        return find_inode_fullpath(ptr+1,tempent.st_ino, fullpath, inodehash);
       }
     }
