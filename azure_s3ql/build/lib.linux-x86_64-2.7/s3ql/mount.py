...
#Jiahong on 11/5/12: Do not check the max_cache_entries. Should do this by product.
    ...
    suggested_max_cache_entries = int(memory / 2048)

    if options.max_cache_entries > suggested_max_cache_entries:
        options.max_cache_entries = suggested_max_cache_entries
    ...
    try:
        ...
        log.info("Retrieve Metadata: connect to cloud failed, will retry after 5 seconds")
        time.sleep(5)
        while True:
            try:
                count = count + 1
                ...
                if count >= 3:
                    log.error("Retrieve Metadata failed, re-connect to cloud exceeds 3 times")
    ...
    metadata_upload_thread = MetadataUploadThread(bucket_pool, param, db,
                                                  options.metadata_upload_interval, var_container, cachepath)
    block_cache = BlockCache(bucket_pool, db, cachepath + '-cache',
                             options.cachesize * 1024, var_container, options.max_cache_entries, options.nodelay)
    ...
    with bucket_pool() as bucket:
        ...
        param['seq_no'] = seq_no  # Modified by Jiahong on 8/27/13: Always force the seq_no to be equal to the one stored on swift
        ...
        else:  # Modified by Jiahong on 8/26/13: Will always try to update the remote metadata to prevent
               # data inconsistency due to Swift consistency window
            cycle_metadata(bucket)
            param['last-modified'] = time.time()

            log.info('Dumping metadata...')
            tempdir = cachepath + '.tmpdir'
            if not os.path.exists(tempdir):
                os.mkdir(tempdir);
            fh = tempfile.TemporaryFile(dir=tempdir)
            dump_metadata(db, fh)
            def do_write(obj_fh):
                fh.seek(0)
...
# Modified by Jiahong on 8/26/13: Comment out the part where s3ql will ignore local meta updates
#        else:
...
#                os.rename(name, name + '.0')
    ...
    if options.profile:
        ...
        if not os.path.exists(tempdir):
            os.mkdir(tempdir);
        tmp = tempfile.NamedTemporaryFile(dir=tempdir)
...
def determine_threads(options):
    ...
    if cores == -1 or memory == -1:
        ...
        threads = min(int((memory / 2) // mem_per_thread), 100)
        if threads > 0:
...
def get_metadata(bucket, cachepath):
    ...
    if not db:
        def do_read(fh):
            ...
            if not os.path.exists(tempdir):
                os.mkdir(tempdir);
            tmpfh = tempfile.TemporaryFile(dir=tempdir)
...
def parse_args(args):
    ...
    parser.add_argument("--nodelay", action="store_true", default=False,
                      help='Disable delaying cache upload if FS read / write is in progress. '
                           '(default: %(default)s)')
...
class MetadataUploadThread(Thread):
    ...
    def __init__(self, bucket_pool, param, db, interval, var_container, cachepath):
            ...
            for (inode_id,block_id) in self.block_cache.opened_entries:
                ...
                if len(local_cache_to_close) > 0:
                    if inode_id in local_cache_to_close:
                        with llfuse.lock:
                            try:
                                if self.block_cache.entries[(inode_id,block_id)].isopen is True and (inode_id, block_id) not in self.block_cache.in_transit:
                                    ...
                                    log.debug('Skipping block (%d, %d), file closed or in transit?' % (inode_id,block_id))
                                do_nothing = False
                            except:
        ...
        while not self.stop_event.is_set():
            ...
            if self.block_cache.do_upload or self.block_cache.forced_upload or self.block_cache.snapshot_upload:
                ...
                for el in self.block_cache.entries.values_rev():
                    ...
                    if ((time.time() - self.block_cache.last_readwrite_time) < 10 and self.block_cache.nodelay is False):
