...
class Fsck(object):
    ...
    def check(self):
        ...
        try:
            ...
            #Jiahong: Disabling flushing the cache to backend at fsck 
            #self.check_cache()
            #Jiahong: Call a new function to list cached blocks
...
#                    log.error('This should not happen, please report a bug.')
    ...
    def check_cached_blocks(self):
        """List the cached inode-blockno pairs"""

        log.info("Listing cached objects...")
    ...
    def check_cache(self):
        ...
        for filename in os.listdir(self.cachedir):
            ...
            #Jiahong: This test assumes that the upload process after the changed block is inserted into the database will be completed before an error
            # and this assumption might be false as perform_write can be incomplete 
            try: 
            ...
            for inode_id, cached_max_size in self.inode_size_cache.iteritems():
                ...
                if size_old < cached_max_size and size < cached_max_size:
                    ...
                    self.log_error("Size of inode %d (%s) does not agree with number of blocks (in cache), "
                               "setting from %d to %d",
                               inode_id, get_path(inode_id, self.conn), size_old, cached_max_size)
                    self.conn.execute("UPDATE inodes SET size=? WHERE id=?", (cached_max_size, inode_id))
...
#Trace and mod this code so that files with blocks in cache (but dirty) will not be moved to lost+found
    def check_blocks_obj_id(self):
        ...
        for (block_id, obj_id) in self.conn.query('SELECT blocks.id, obj_id FROM blocks LEFT JOIN objects '
            ...
            for (inode, blockno) in self.conn.query('SELECT inode, blockno FROM inode_blocks WHERE block_id = ? ',
                                            (block_id,)):
                # Jiahong (4/27/12): New code for skipping files that have this data block in cache
                if (inode, blockno) in self.cached_blocks:
                    continue
    ...
    def check_inode_blocks_block_id(self):
        ...
        to_delete = list()
        #Jiahong (4/27/12): Modifying the following database query to return both the inode and blockno 
        for (rowid, block_id, inode, blockno) in self.conn.query('SELECT inode_blocks.rowid, block_id, inode, blockno FROM inode_blocks '
        ...
        try:
            ...
            for (obj_id,) in self.conn.query('SELECT * FROM missing'):
                ...
                for (id_, blockno) in self.conn.query('SELECT inode, blockno FROM inode_blocks JOIN blocks ON block_id = id '
                                              'WHERE obj_id=? ', (obj_id,)):
                    # Jiahong (4/27/12): New code for skipping files that have this data block in cache
                    if (id_, blockno) in self.cached_blocks:
                        continue
...
#Commented out the following two lines so that we do not remove cache contents even after fsck.s3ql
    ...
    try:
        ...
        log.info("Cycle Metadata: connect to cloud failed, will retry after 5 seconds")
        time.sleep(5)
        while True:
            try:
                count = count + 1
                ...
                if count >= 3:
                    log.error("Cycle Metadata failed, re-connect to cloud exceeds 3 times")
                    raise QuietError(str(exc))
                else:
                    log.info("Cycle Metadata: connect to cloud failed, will retry after 5 seconds")
                    time.sleep(5)
