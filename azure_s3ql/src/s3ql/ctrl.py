...
def parse_args(args):
    ...
    subparsers.add_parser('upload-meta', help='Upload metadata',
                          parents=[pparser])
    subparsers.add_parser('uploadon', help='start upload dirty file system cache',
                          parents=[pparser])
    ...
    subparsers.add_parser('writeoff', help='disallow write operation to file system',
                          parents=[pparser])
    subparsers.add_parser('quotaon', help='enable quota limit',
                          parents=[pparser])
    subparsers.add_parser('quotaoff', help='disable quota limit',
                          parents=[pparser])
...
def main(args=None):
    ...
    if options.action == 'uploadon':
        llfuse.setxattr(ctrlfile, 'uploadon', 'dummy')
    ...
    if options.action == 'quotaon':
        llfuse.setxattr(ctrlfile, 'quotaon', 'dummy')

    if options.action == 'quotaoff':
        llfuse.setxattr(ctrlfile, 'quotaoff', 'dummy')
