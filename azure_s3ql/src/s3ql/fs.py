    ...
    def setxattr(self, id_, name, value):
        ...
        if id_ == CTRL_INODE:
            if name == b's3ql_flushcache!':
                ...
                self.cache.use_quota = True
            elif name == 'quotaoff':
                self.cache.use_quota = False
                self.cache.do_write = True
            elif name == 'copy':
...
#Added by Jiahong Wu: New function that returns status of local cache
    def extstat_cache(self):
        ...
        # yuxun, 2014/5/7
        # pack a value to show if the limit of quota was actived
        if (self.cache.use_quota):
            use_quota = 1
        else:
            use_quota = 0
    ...
    def statfs(self):
        ...
        # Jiahong 2013/9/23
        # Revert block size to fixed to avoid integer overflow problem in cython.

        fs_size = max(self.cache.quota_size, data_size)

        # If system size is too large then need to expand block size. Default is 4K bytes.
        if ((fs_size // (2**30)) > 4096):
            stat_.f_frsize = fs_size // (2**30)
        else:
            stat_.f_frsize = 4096
    ...
    def open(self, id_, flags):
        ...
        if id_ in self.cache.cache_to_close:
            try:
                ...
                log.debug('Not found in cache_to_close. Perhaps already removed')
