...
class BlockCache(object):
    ...
    def __init__(self, bucket_pool, db, cachedir, max_size, var_container, max_entries=768, nodelay=False):
        ...
        self.quota_size = 100 * (1024 ** 4)  #Jiahong: Will we be able to set quota_size now? If not, it might be better to set this to 10T on the repository?
        ...
        self.nodelay = nodelay  # Jiahong (2/11/14): Add flag for whether to delay cache flush if read/write is in progress. Default is False
    ...
    def check_quota(self):
        ...
        if self.use_quota:
            ...
            if self.value_cache["fs_size"] >= int( self.quota_size * 0.95 ):
                self.quota_size = int(self.quota_size * 1.2)
                self.do_write = True
...
#Jiahong (5/4/12): Implemented a mechanism for labeling partially downloaded block objects. Such objects are removed during fsck
        ...
        #  Edited on 10/9/13 to handle closing right after downloading.
