...
deploy_meta = {
    ...
    "name": "Openstack Diablo 2011 Sep (80% fail version)",
    "base_os": ["ubuntu", "centos"],
    ...
    "user_config":{
        ...
        "bool": {
            "description": "You realize this is just a sample?",
            "type": "bool",
            "required": True,
            "default": True,
            "order": 2
...
"name": "Openstack Diablo 2011 Sep",
...
# sub-classing WorkerTask and override run() to define worker task
@task_class
class ZoneDeploy(WorkerTask):
    ...
    def run(self, *args, **kwargs):
        ...
        while progress < 80:
            ...
            time.sleep(1)
            progress += 10
        zone_deploy.update_progress(80, 'deployment failed!')
        raise Exception('80% failure')
        return 'failed'
