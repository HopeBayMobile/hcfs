...
class Test_formatNonRootDisks:
    ...
    def test_FormatOperation(self):
        ...
        if self.__testable == False:
            ...
            nose.tools.ok_(len(result[1]) == len(self.__disk) - 1,\
                           "The number of formatted disks returned by formatNonRootDisks() is not correct!")
            ...
            for item in self.__disk:
                ...
                nose.tools.ok_(not os.path.exists("%s/%s" % (self.__mount_dir, self.__mark)),\
                               "The format operation performed by formatNonRootDisks() failed!")
...
class Test_mountSwiftDevice:
    ...
    def test_MountOperation(self):
        ...
        nose.tools.ok_(os.path.ismount("/srv/node/%s%d" % (self.__prefix, self.__deviceNum)),\
                       "Failed to mount Swift device by mountSwiftDevice()!")
...
class Test_getLatestFingerprint:
    ...
    def test_NoFingerprint(self):
        ...
        nose.tools.ok_(result == None or result == [] or result == "" or result == {},\
                       "The fingerprint returned by getLatestFingerprint() does not exist!")
