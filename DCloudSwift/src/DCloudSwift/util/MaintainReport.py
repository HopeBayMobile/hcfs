...
class MaintainReport():
    ...
    def query(self, target = None):
        ...
        if target is None:
            ...
            result = self.db.query_maintenance_backlog_table('target = %s' % target)
        for row in result:
            print row
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        if 'test' == sys.argv[1]:
            ...
            row = db.add_maintenance_backlog('node_missing', '192.168.2.30', json.dumps(diskReserve), json.dumps(diskReplace), timestamp)
    main(DBFile)