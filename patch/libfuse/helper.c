...
static int fuse_mount_common(const char *mountpoint,
                             struct fuse_args *args, int fd1)
{
        struct fuse_chan *ch;
        int fd;
...
int fuse_mount(const char *mountpoint, struct fuse_args *args, int fd)
{
	return fuse_mount_common(mountpoint, args, fd);
}
			       ...
			       int compat)
{
	...
	ch = fuse_mount_common(*mountpoint, &args, -1);
	if (!ch) {
