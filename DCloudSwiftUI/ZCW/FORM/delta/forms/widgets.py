...
class CIDRAddressInput(TextInput):
    ...
    def value_from_datadict(self, data, files, name):
        ...
        data = '.'.join(input_list[0:4]) + '/' + input_list[4]
        if any(input_list):
            return data
        else:
            return None
    ...
    def render(self, name, value, attrs=None):
        ...
        cidr = [''] * 5
        if value:
            ip, cidr[4] = value.split('/')
            cidr[0:4] = ip.split('.')

        # Create inputs for CIDR address.
        input_list = []
        for item in cidr:
            final_attrs['value'] = force_unicode(self._format_value(item))
            input_list.append(u'<input%s />' % flatatt(final_attrs))
        render_mark = u'/'.join(['.'.join(input_list[0:4]), input_list[4]])
