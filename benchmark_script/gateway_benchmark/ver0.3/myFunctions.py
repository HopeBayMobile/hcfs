...
def clean_DB():   # delete all tables in DB
	...
	cursor = db.cursor()
	qstr = "DROP TABLE File_List"  
	cursor.execute(qstr)
	qstr = "DROP TABLE Folder_List"  
	cursor.execute(qstr)
	qstr = "DROP TABLE Operation_Log"  
	cursor.execute(qstr)
...
def fnc_read_a_file(tmp_dir):
	...
	try:
		...
		if a>0:
			...
			, success=0, verify_checksum="+ str(ver_cs) + ", error_msg='" + msg + "' WHERE note='"  
