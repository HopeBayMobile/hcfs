...
public class StorageSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    public void onCreate(Bundle icicle) {
        ...
        mCloudSummary = new StorageSummaryPreference(context);
    ...
    private void refresh() {
        ...
        for (VolumeInfo vol : volumes) {
            ...
            } else if (vol.getType() == VolumeInfo.TYPE_HCFS) {
                if (mStorageTera.hcfsEnabled()) {
        ...
        for (VolumeRecord rec : recs) {
            if (rec.getType() == VolumeInfo.TYPE_PRIVATE
                ...
                if (mStorageTera.hcfsEnabled()) {
                    mCloudCategory.addPreference(pref);
                }
            }
        }
        ...
        if (mStorageTera.hcfsEnabled()) {
            try {
                ...
                final BytesResult cloud_result = Formatter.formatBytes(getResources(), cloud_used, 0);
                mCloudSummary.setTitle(TextUtils.expandTemplate(getText(R.string.storage_size_large),
                        cloud_result.value, cloud_result.units));
                mCloudSummary.setSummary(getString(R.string.storage_volume_used_total,
                        Formatter.formatFileSize(context, quota)));
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference pref) {
        ...
        if (pref instanceof StorageVolumePreference) {
            ...
            if (vol.getType() == VolumeInfo.TYPE_PRIVATE) {
                ...
                final Bundle args = new Bundle();
                args.putString(VolumeInfo.EXTRA_VOLUME_ID, VolumeInfo.ID_HCFS);
                args.putString(VolumeInfo.EXTRA_PRIVATE_VOLUME, VolumeInfo.ID_PRIVATE_INTERNAL);
                startFragment(this, HCFSVolumeSettings.class.getCanonicalName(),
                        -1, 0, args);
            }
