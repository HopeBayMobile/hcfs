...
/*TODO: use nanosleep or signal to replace sleep for 1 second*/
  ...
  while(TRUE)  /*TODO: Perhaps need to change this flag to allow terminating at shutdown*/
   {
    ...
    for(count=0;count<MAX_UPLOAD_CONCURRENCY;count++)
     {
      if ((upload_thread_control.upload_threads_in_use[count]!=0) && (upload_thread_control.upload_threads[count].is_block == TRUE))
       {
...
void init_upload_control()
 {
  ...
  for(count=0;count<MAX_UPLOAD_CONCURRENCY;count++)
   {
    hcfs_init_swift_backend(&(upload_curl_handles[count]));
   }
...
void do_block_sync(ino_t this_inode, long block_no, CURL_HANDLE *curl_handle, char *filename)
 {
  ...
  hcfs_swift_put_object(fptr,objname, curl_handle);
  fclose(fptr);
  return;
 }
      ...
      if (inode_to_check !=0)
       {
        ...
        read_super_inode_entry(inode_to_sync,&tempentry);
