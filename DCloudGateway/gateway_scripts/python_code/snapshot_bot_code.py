...
def execute_take_snapshot():
    ...
    while True:
        try:
            ...
            if ret_val["code"] != 200:
                log.debug('Failed to get snapshot task. Response: %s' % response)
                os.system('sudo rm -rf %s' % snapshot.snapshot_tag)
                break
            ...
            if not os.path.exists(src_folder):
                ...
                if ret_val["code"] != 200 or response["statuscode"] != 200:
                    log.debug('Failed to report snapshot task status (id: %s)' % (sid))
                time.sleep(3)
                continue
            ...
            if po.returncode != 0:
                ...
                if not os.path.exists(snapshot_statistics):
                    ...
                    response = json.loads(ret_val["response"])
                    if ret_val["code"] != 200 or response["statuscode"] != 200:
                        log.debug('Failed to report snapshot task status (id: %s). Response: %s' % (sid, response))
                    time.sleep(3)
                    continue
