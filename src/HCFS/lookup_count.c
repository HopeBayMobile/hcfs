				...
				int amount, char d_type)
{
	...
	write_log(10, "Debug lookup increase for inode %ju, amount %d\n",
			(uintmax_t)this_inode, amount);
			...
			int amount, char *d_type, char *need_delete)
{
	...
	write_log(10, "Debug lookup decrease for inode %ju, amount %d\n",
			(uintmax_t)this_inode, amount);
...
int lookup_markdelete(LOOKUP_HEAD_TYPE *lookup_table, ino_t this_inode)
{
	...
	write_log(10, "Debug lookup markdelete for inode %ju\n",
...
int lookup_destroy(LOOKUP_HEAD_TYPE *lookup_table, MOUNT_T *tmpptr)
{
	...
	for (count = 0; count < NUM_LOOKUP_ENTRIES; count++) {
		...
		while (ptr != NULL) {
			write_log(10, "Debug check delete %ju\n",
				(uintmax_t)ptr->this_inode);
