...
void dump_sb_entries()
{
	...
	fprintf(dump_fptr, "Cache Size - %ld, Dirty Cache Size - %ld, Unpin "
			   "Dirty Size - %ld\n\n",
	...
	fprintf(dump_fptr, "Recovery sb is ongoing - %s, "
		...
		(sys_super_block->sb_recovery_meta.is_ongoing) ? "TRUE"
							       : "FALSE",
		sys_super_block->sb_recovery_meta.start_inode,
		sys_super_block->sb_recovery_meta.end_inode, buf);

	fprintf(dump_fptr, "SB HEAD:\n");
	fprintf(dump_fptr,
		"First Dirty - %ld, Last Dirty - %ld, Num Dirty - %ld\n\n",
	...
	while (start_inode < end_inode) {
		...
		for (idx = 0; idx < num_entry_handle; idx++) {
			sbentry2 = (sb_entry_arr + idx);
			fprintf(dump_fptr, "Ino %ld,\t\tstatus %d,\t\tpin "
					   "status %d,\t\tprev %ld,\t\tnext "
					   "%ld\n",
				sbentry2->this_index, sbentry2->status,
				sbentry2->pin_status, sbentry2->util_ll_prev,
	...
	while (forward != 0 && backward != 0) {
		...
		if (backward != 0) {
			...
			if (ret_code < 0) {
				...
				read_size = fread(large_buf, 1, 1024 * 150,
						  dirty_backward);
				rewind(dirty_backward);
